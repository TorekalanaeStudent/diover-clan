...
import com.obedoza.app.dto.OrderSummary;
...
public class OrderServiceTest {
    ...
    // Test 9 - DIP proof: works through interface not implementation
    @Test
    void shouldWorkThroughInterface() {
        OrderService service = orderService; // typed as interface
        when(orderRepository.findAll()).thenReturn(Collections.emptyList());
    ...
    // Test 10 - Immutability: OrderNotFoundException preserves state
    @Test
    void shouldPreserveOrderIdInException() {
        when(orderRepository.findById(99L)).thenReturn(Optional.empty());

        OrderNotFoundException ex = assertThrows(OrderNotFoundException.class,
                () -> orderService.getOrder(99L));
        assertEquals(99L, ex.getOrderId());
    }

    @Test
    void shouldCreateImmutableOrderSummary() {
        // OOP: Immutability - record fields cannot be changed after creation
        OrderSummary summary = new OrderSummary(1L, "Alice", 25.0);

        assertEquals(1L, summary.id());
        assertEquals("Alice", summary.customerName());
        assertEquals(25.0, summary.total());
    }

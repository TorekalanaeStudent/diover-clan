...
import com.caneda.springboot.controller.MeController_Caneda;
...
public class MeController {
    ...
    @Autowired
    private MeController_Caneda controller;

    private MockMvc mockMvc = standaloneSetup(controller).build();

    @Test
    public void testGetMe() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/me")).andExpect(status().isOk()).andExpect(content().contentType("application/json")).andExpect(jsonPath("$.name").exists());
